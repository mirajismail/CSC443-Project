...
void testMemtableFlush() {
    ...
    KVStore<int, int> store{3}; // small size to trigger flush
...
void testScanAcrossSSTables() {
    ...
    KVStore<int, int> store{3}; // small memtable to create multiple ssts
...
void cleanup() {
    ...
    system("rm -rf test_basic* test_flush* test_update* test_scan* test_multi_sst* test_strings*");
}
